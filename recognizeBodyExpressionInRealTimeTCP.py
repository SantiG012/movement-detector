...
# Host 
HOST = "172.16.206.255" # Direction ip computadora servidor
...
# Method send message and landmarks
def sendMessageAndLandmarks(landmarks, message):
    # Create socket
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        # Connect to server
        s.connect((HOST, PORT))
        # Send data
        s.sendall((landmarks + " " + message).encode('utf-8'))
        # Print data send
        print("Data send: " + landmarks + " " + message)
                ...
                if leftWrist.y < 0.2 and rightWrist.y < 0.2:
                    ...
                    if not handsUpSent:
                        ...
                        if handsUp:
                            ...
                            sendMessageAndLandmarks(landmarks, "HandsUp")
                ...
                if left_hip.y > 0.8 and right_hip.y is not None and right_hip.y > 0.8 and results.pose_landmarks.landmark[12].y < 0.5:
                    ...
                    if not handsDownSent:
                        ...
                        if handsDown:
                            ...
                            # Send Message using socket
                            sendMessageAndLandmarks(landmarks, "HandsDown")                            
